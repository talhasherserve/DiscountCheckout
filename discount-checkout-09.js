...
let CustomCheckoutBaseUrl = null;
...
async function loadCustomCheckoutBaseUrl() {
    if (CustomCheckoutBaseUrl) return CustomCheckoutBaseUrl;

    const res = await fetch(
        `${url}GiftShipping/CustomCheckoutBaseUrl`,
        {
            method: 'GET',
            headers: {
    ...
    // adjust for ResultDTO structure
    CustomCheckoutBaseUrl = data.data || data;

    return CustomCheckoutBaseUrl;
}
...
function setupCheckoutElement(el) {
    ...
    el.addEventListener('click', (event) => {
        ...
        //     event.preventDefault();
        //     event.stopPropagation();
        //      window.location.href = `${CustomCheckoutBaseUrl}?shopName=${shop}`;
        //     //  window.location.href = `${CustomCheckoutBaseUrl}?shopName=${encodeURIComponent(shop)}`;
...
async function handleInternationalCheckout() {
    ...
    try {
        ...
        disableCheckoutElements(true, 'Redirecting...');
        ...
        const selectedCountry =
            sessionStorage.getItem('giftShipSelectedCountry');
        ...
        const response = await fetch(
            `${url}GiftShipping/CustomCheckout`,
            {
                method: 'POST',
                headers: {
        ...
        const baseUrl = await loadCustomCheckoutBaseUrl();

        window.location.href =
            `${baseUrl}?shopName=${encodeURIComponent(shop)}&cartToken=${encodeURIComponent(cart.token)}`;

    } catch (error) {

        console.error('International checkout failed:', error);
...
//         // const cart = await cartResponse.json();
//         // const cartId = encodeURIComponent(cart.token);
//          const selectedCountry = sessionStorage.getItem('giftShipSelectedCountry');
...
//     selectedCountry
// };
//         // Call backend custom checkout API
//         const response = await fetch(
//             `${url}GiftShipping/CustomCheckout`,
//             {
//                 method: 'POST',
...
//         if (!response.ok) {
//             throw new Error('Custom checkout API failed');
//         }

//         // Optional: use backend response later if needed
//         const data = await response.json();

//         const baseUrl = await loadCustomCheckoutBaseUrl();

//         window.location.href =
//             `${baseUrl}?shopName=${encodeURIComponent(shop)}&cartId=${encodeURIComponent(cartId)}`;
...
//         // alert('Unable to proceed to international checkout.');
//     }
// }
